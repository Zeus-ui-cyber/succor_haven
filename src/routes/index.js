...
// ── Session Room · teaching materials upload config ───────────────────────
// Stored outside src/ at project-root/uploads/session-files. Per the
// spec, teachers can upload PDF/PowerPoint/Word/images/video during a
// live session — the widest allowed-type list of any upload in this app,
// hence the larger 100MB ceiling (video).
const sessionFileDir = path.join(__dirname, "..", "..", "uploads", "session-files");
...
const uploadSessionFile = multer({
  ...
  limits: { fileSize: 100 * 1024 * 1024 }, // 100MB — video is allowed here
  fileFilter: (req, file, cb) => {
    if (!ALLOWED_SESSION_FILE_TYPES.includes(file.mimetype)) {
      return cb(
        new Error("Only images, PDF, DOC/DOCX, PPT/PPTX, or video files are allowed."),
      );
    }
    cb(null, true);
  },
...
router.patch("/sessions/:id/end", authenticate, sessionsController.endSession);
router.get("/sessions/:id/chat", authenticate, sessionsController.getChatHistory);
router.get("/sessions/:id/notes", authenticate, sessionsController.getMyNotes);
router.patch("/sessions/:id/notes", authenticate, sessionsController.saveMyNotes);
router.get("/sessions/:id/files", authenticate, sessionsController.getSessionFiles);
router.post(
  "/sessions/:id/files",
  authenticate,
  uploadSessionFile.single("file"),
  sessionsController.uploadSessionFile,
);
